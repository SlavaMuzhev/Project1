...
def mask_account_card(card_details_or_account: str) -> str:
    ...
    number = "".join(list(filter(lambda c: c.isdigit(), card_details_or_account)))
    card_type_or_account = "".join(list(filter(lambda c: not c.isdigit(), card_details_or_account)))
